...
function getToledoChess() {
	...
	function triggerAI()
	{
		var ax = compute(0,0,0,21,pawn2SquareMove,2); // 2 = ply
		var bx = compute(0,0,0,21,pawn2SquareMove,1);
		if(ax > 70000) {
			console.debug('Black won!');
			callbacks.checkmateCallback(-1); // Black
		}
		clickLock=false;
	}
	...
	function OnClick(fieldID) {
		...
		if (curPieceCode > 8) {
			...
			if ((curPieceCode & 7) == 1 & (moveTo < 29 | moveTo > 90))
				...
				curPieceCode = 11 ^ curPlayer; // just always choose a queen
			// verify player move and execute it
			if(compute(0,0,0,21,pawn2SquareMove,0) > 70000) {
				console.debug('White won!');
				callbacks.checkmateCallback(1); // White
			}
